...
export class ToResultaatItemPipe implements PipeTransform {
    transform<T extends SGeldendResultaat>(resultaat: ToetsResultaat<T>, isAlternatiefNiveau: boolean): ResultaatItem {
        const teltNietMee: TeltNietMeeType | undefined = resultaat.geldendResultaat.weging === 0 ? 'Dit cijfer telt niet mee' : undefined;
        ...
        return {
            ...
            details: {
                ...
                weging: bepaalWeging(resultaat.geldendResultaat.dossierType, resultaat.wegingVoortgang, resultaat.wegingExamen),
                afwijkendeWegingExamen: bepaalAfwijkendeWegingExamen(
                    resultaat.geldendResultaat.dossierType,
                    resultaat.wegingVoortgang,
                    resultaat.wegingExamen
                ),
...
function bepaalWeging(dossierType: DossierType, wegingVoortgang?: string, wegingExamen?: string): string {
    if (!wegingVoortgang) {
        return wegingExamen ?? '';
    }

    return wegingVoortgang;
}

function bepaalAfwijkendeWegingExamen(dossierType: DossierType, wegingVoortgang?: string, wegingExamen?: string): string | undefined {
