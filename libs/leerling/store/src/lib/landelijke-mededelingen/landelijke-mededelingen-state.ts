...
export const LANDELIJKE_MEDEDELINGEN_STATE_TOKEN = new StateToken<SLandelijkeMededelingenModel>('landelijkeMededelingen');
...
export class LandelijkeMededelingenState extends AbstractState {
    ...
    override switchContext(ctx: StateContext<SLandelijkeMededelingenModel>) {
        return ctx.setState(
            patch<SLandelijkeMededelingenModel>({ accounts: this.ontdubbelLandelijkeMededelingen(ctx.getState().accounts) })
        );
    }
